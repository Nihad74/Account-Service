...
public class RoleService {
    ...
    public ResponseEntity<?> updateUserRole(UpdateRole updateRole){
        ...
        log.error("updateRole: {}", updateRole);
        ...
        if(!updateRole.getRole().equals("ADMINISTRATOR") && !updateRole.getRole().equals("ACCOUNTANT")
                && !updateRole.getRole().equals("USER") && !updateRole.getRole().equals("AUDITOR")){
        ...
        if(updateRole.getOperation().equals("GRANT")){
            ...
            if(updateRole.getRole().equals("ADMINISTRATOR") && !user.getRoles().contains(new Roles("ROLE_ADMINISTRATOR"))){
                ...
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The user cannot combine administrative and business roles!");
            }else if (updateRole.getRole().equals("AUDITOR") && user.getRoles().contains(new Roles("ROLE_ADMINISTRATOR"))){
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The user cannot combine administrative and business roles!");
            }

            securityEventRepository.save(new SecurityEvent(LocalDate.now(), "GRANT_ROLE", subject.getUsername().toLowerCase(),
                    "Grant role " + updateRole.getRole() + " to " + user.getEmail().toLowerCase(), "/api/admin/user/role"));
            ...
            securityEventRepository.save(new SecurityEvent(LocalDate.now(), "REMOVE_ROLE", subject.getUsername().toLowerCase(),
                    "Remove role " + updateRole.getRole() + " from " + user.getEmail().toLowerCase(), "/api/admin/user/role"));
            }
    ...
    public ResponseEntity<?> deleteUser (String email) {
        ...
        String username = authentication.getName();
        UserAdapter subject = (UserAdapter) paymentService.loadUserByUsername(username);
        securityEventRepository.save(new SecurityEvent(LocalDate.now(), "DELETE_USER", subject.getUsername().toLowerCase(),
                deletedUser.getEmail().toLowerCase(), "/api/admin/user/delete"));
